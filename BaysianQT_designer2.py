...
class Mainwindow():
    ...
    def Fileselect(self):
        ...
        data_processed1 = data_processed[['temp1pro','temp2pro','temp3pro','temp4pro','UU','UI','RPHSU','RPHSI','underU','overU','overI','overP','failurePh','loseU','underPF','RP']]
        search_result_model = PdTable(data_processed)
...
class ChildWindow():
    ...
    def methodselect(self):
        ...
        if selectedbutton == -2:
            ...
            '''
            #贝叶斯网络运算
            scoring_method = K2Score(data=data_processed1)
            est = HillClimbSearch(data=data_processed1)
            ...
            model_struct.add_nodes_from(['WrongOpt','BrkbyHu','UnMaint','OldElec','DesFau','DesFau','Undefend','ComMat','FlamMat', 'fire'])
            model_struct.add_edge('WrongOpt','fire')
            model_struct.add_edge('BrkbyHu', 'fire')
            model_struct.add_edge('UnMaint', 'fire')
            model_struct.add_edge('OldElec', 'fire')
            model_struct.add_edge('DesFau', 'fire')
            model_struct.add_edge('Undefend', 'fire')
            model_struct.add_edge('ComMat', 'fire')
            model_struct.add_edge('FlamMat', 'fire')
            model_struct.add_edge('temp4pro', 'fire')
            ...
            model_struct = BayesianModel([('UU', 'temp1pro'),
                                          ('UI', 'temp1pro'),
                                          ('RPHSU', 'temp1pro'),
                                          ('RPHSI', 'temp1pro'),
                                          ('underU', 'temp1pro'),
            ...
            estimated_model = est.estimate(variant='orig', max_cond_vars=3)
            model_struct = BayesianModel(estimated_model.edges())
            model_struct.fit(data=data_processed, estimator=MaximumLikelihoodEstimator)
...
class ChildWindow2():
    def __init__(self):
        ...
        self.ui.buttonGroup.buttonClicked.connect(self.inferselect)
        self.inselectedbutton = 'No'
        ...
        self.ui.pushButton_2.clicked.connect(self.inference)
        self.ui.pushButton.clicked.connect(self.exit)

    def inferselect(self):
        self.inselectedbutton = self.ui.buttonGroup.checkedId()
        'print(self.inselectedbutton)'
    ...
    def modelinference(self):
        ...
        '''
            ...
            model_infer = BeliefPropagation(model_struct)
            if self.ui.TargetcomboBox.currentText() in ['temp1pro','temp2pro','temp3pro','temp4pro']:
                q_after = model_infer.query(variables=[self.ui.TargetcomboBox.currentText()], evidence={self.ui.EvidencecomboBox.currentText(): self.ui.IntervalcomboBox.currentText()})
            else:
                q_after = model_infer.query(variables=[self.ui.TargetcomboBox.currentText()], evidence={self.ui.EvidencecomboBox.currentText(): int(self.ui.IntervalcomboBox.currentText())})
        self.gv_visual_data_content3.axes.bar(q_before.state_names[self.ui.TargetcomboBox.currentText()],
    ...
    def inference(self):
        ...
        self.gv_visual_data_content3 = MyFigureCanvas(width=self.ui.Targetafter.width() / 101,
                                                      height=self.ui.Targetafter.height() / 101,
                                                      xlabel=self.ui.TargetcomboBox.currentText())
        ...
        if self.inselectedbutton == -2:
            ...
            model_infer = BeliefPropagation(model_struct)
            if self.ui.EvidencecomboBox.currentText() in ['temp1pro','temp2pro','temp3pro','temp4pro']:
                q_after = model_infer.query(variables=[self.ui.TargetcomboBox.currentText()], evidence={self.ui.EvidencecomboBox.currentText(): self.ui.IntervalcomboBox.currentText()})
            else:
                q_after = model_infer.query(variables=[self.ui.TargetcomboBox.currentText()], evidence={self.ui.EvidencecomboBox.currentText(): np.int(self.ui.IntervalcomboBox.currentText())})
        self.gv_visual_data_content3.axes.bar(q_before.state_names[self.ui.TargetcomboBox.currentText()],
                                              q_after.values, label=self.ui.TargetcomboBox.currentText())
        self.graphic_scene = QGraphicsScene()  # 创建一个QGraphicsScene
        self.graphic_scene.addWidget(self.gv_visual_data_content3)  # 把图形放到QGraphicsScene中，注意：图形是作为一个QWidget放到放到QGraphicsScene中的
